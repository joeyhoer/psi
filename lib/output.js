...
module.exports = (parameters, response) => {
  return Promise.resolve().then(() => {
    ...
    if (!response.ruleGroups.USABILITY.pass) {
      throw new Error('Page is not mobile ready');
    }

    if (parameters.download) {
      const file = 'screen.jpg';
      if (typeof parameters.download == 'string') {
        file = parameters.download;
      }
      // response.screenshot.mime_type == image/jpeg
      return fs.writeFileSync(file, response.screenshot.data, 'base64');
    }
  });
};
